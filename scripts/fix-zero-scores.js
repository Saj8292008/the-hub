#!/usr/bin/env node
/**
 * Fix Zero Scores - Re-score all listings with deal_score = 0 or NULL
 * 
 * Usage:
 *   node scripts/fix-zero-scores.js              # rescore all zero/null scores
 *   node scripts/fix-zero-scores.js --dry-run    # score but don't write to DB
 *   node scripts/fix-zero-scores.js --limit 100  # only first 100 listings
 */

require('dotenv').config();
const supabase = require('../src/db/supabase');
const { scoreDeal, clearMarketPriceCache } = require('../src/services/deal-scorer');
...
async function main() {
  console.log('═══════════════════════════════════════════════');
  console.log('  Fix Zero Scores - Deal Rescorer v3.0');
  console.log('═══════════════════════════════════════════════');
  ...
  // Fetch listings with NULL or 0 scores
  console.log('📥 Fetching listings with score = 0 or NULL...');
  ...
  while (allListings.length < LIMIT) {
    ...
    const { data, error } = await supabase.client
      .from('watch_listings')
      .select('*')
      .or('deal_score.is.null,deal_score.eq.0')
  ...
  console.log(`✅ Fetched ${allListings.length} listings with zero/null scores\n`);

  if (allListings.length === 0) {
    console.log('✨ No listings to score! All listings already have scores.');
    process.exit(0);
  }
  ...
  console.log('\n  🏆 Top 10 Deals (from previously unscored):');
  console.log('  ─────────────────────────────────────────');
